          ...
          {results && results.length > 0 ? (
            <div className="results-grid">
              {results.map((card, index) => (
                <div className="card-result" key={`${card.set_name}-${card.collection_number}-${index}`}>
                  <img 
                    src={card.normal_img} 
                    alt={card.name}
                  ...
                  <div className="card-details">
                    <h3>{card.name}</h3>
                    <p>{card.set_name} • #{card.collection_number}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <p className="no-results">
              {initialQuery ? 'No results found. Try a different search term.' : 'Enter a search term to find cards.'}
            </p>
          )}
